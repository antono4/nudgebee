...
export default function SignInV2({ providers, samlEnabled, tier }: any) {
  ...
  const handleCredentialsSubmit = async (provider: any, ev: any) => {
    ...
    const result = await signIn(provider.id, { username: credsUsername.trim().toLowerCase(), password: credsPassword, redirect: false });
    if (result?.error) {
      setHelperText((prev) => ({
        ...prev,
  ...
  if (providers == undefined || providers == null || Object.values(providers).length == 0) {
    return (
      <>
        <Head>
          <title>Nudgebee: Login </title>
          <meta property='og:title' content={`NUDGEBEE - Signin`} key='title' />
        </Head>
        <Box
          sx={{
            height: '100vh',
